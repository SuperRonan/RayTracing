...
namespace sys
{
	...
	{
		//////////////////////////////////////
		// converts an floating point image to uchar image
		// applies a simple tone mapping
		//////////////////////////////////////
		template <class floot>
		__global__ void convertImage(uint8_t* d_ucfb, const colors::RGBColor<floot>* d_fb, unsigned int width, unsigned int height)
		{
			...
			if (i < height && j < width)
			{
				colors::RGBColor<floot> pixel = d_fb[index];
				pixel /= (pixel + 1);
				pixel *= 256;
				uint8_t r = pixel[0];
				uint8_t g = pixel[1];
...
#define amask = 0xff000000;
	...
	public:
		...
		*/
		...
		void blit(const colors::RGBColor<floot>* d_fb, unsigned int width, unsigned int height, dim3 const& block_size, dim3 const& grid_size)
		{
			kernel::convertImage << <grid_size, block_size >> > (d_ucfb, d_fb, width, height);
